...
void setup() {
  ...
  Serial.begin(115200);
  while (!Serial);
  Serial.println("LoRa Sender & Receiver - ESP32_DRONE MODULE");
  ...
  Serial2.begin(112500, SERIAL_8N1, 3, 1); // UART on GPIO3 (RX) and GPIO1 (TX)
...
void raspberrypidata() {
  if (Serial2.available()) { // Check if data is available on UART
    ...
    // Send modified LoRa packet based on received data
    LoRa.beginPacket();
    if (receivedData == "SOS detected") {
      LoRa.print("SOS");
    } else {
      LoRa.print(receivedData);
    }
    if (receivedData == "Danger detected") {
      LoRa.print("DANGER");
    } else {
      LoRa.print(receivedData);
    }
